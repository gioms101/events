...
class EventConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        self.movie_id = self.scope['url_route']['kwargs']['pk']
        date = query_params.get("date", [None])[0]
        theater = query_params.get("theater", [None])[0]

        if not all([self.movie_id, date, theater]):
            await self.close()
            return

        self.room_name = f'events_{self.movie_id}_{date}_{theater}'  # This is going to be room name Where user can
                                                                     # select tickets.
    ...
    async def receive(self, text_data=None, bytes_data=None):
        ...
        ticket_status = await self.select_ticket(ticket_id, self.movie_id)
    ...
    def get_user(self, token):
        ...
        try:
            ...
            return User.objects.prefetch_related('selected_tickets').get(id=decoded_token["user_id"])
        except Exception:
            return None

    @database_sync_to_async
    def select_ticket(self, ticket_id, movie_id):

        if self.user.selected_tickets.all().count() > 4:
            return "Can't select more than 4 tickets"

        ticket = Ticket.objects.select_for_update().filter(id=ticket_id, movie_id=movie_id)
        if ticket.exists():
            ticket = ticket.first()
